    ...
    private java.util.List<String[]> buildExtraFieldSpecs(String caseId, int mIdx, CaseConfigManager cfgMgr) {
        ...
        switch (caseId) {
            ...
            case "CTSW102": {
                ...
                if (mIdx == 2 || mIdx == 8) {
                    specs.add(new String[]{"amqp_message_id", "AMQP MESSAGE-ID (empty → REJECT)", ""});
                }
                if (mIdx == 3 || mIdx == 9) {
                    specs.add(new String[]{"creation_time", "AMQP CREATION-TIME (0 → REJECT)", "0"});
                }
                break;
            }
            // ── CTSW103 ──────────────────────────────────────────────────────────
