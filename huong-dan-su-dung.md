...
## 4. Quy trình làm việc (process) và thao tác kiểm thử
Để thực hiện kiểm tra các kịch bản từ CTSW101 tới CTSW116 và tạo báo cáo thử nghiệm, tester sẽ thực hiện các bước như sau:

1. **Khởi tạo và ghi nhận thời gian:**
   - Mở ứng dụng AMHS/SWIM Gateway Test Tool.
   - Bấm vào nút **Start Recording** ở thanh công cụ phía trên cùng của ứng dụng để lưu giờ bắt đầu kiểm tra. Nút này sau đó sẽ chuyển thành dạng **Stop** (hoặc **Stop Recording**).
   - Khi kết thúc toàn bộ quá trình test, bấm lại nút **Stop** để hệ thống ghi nhận giờ kết thúc vào báo cáo.

2. **Cấu hình (Settings):**
   - Tester có thể tùy chỉnh các cài đặt tổng của tool thông qua phần Settings (cấu hình broker, giá trị mặc định, v.v.).

3. **Chọn và thực hiện Test case/Subcase:**
   - Trong cây danh sách bên trái, chọn một case hoặc subcase tương ứng (từ CTSW101 đến CTSW116).
   - Khi chọn một subcase, màn hình sẽ hiển thị:
     - **Mô tả:** Xem được mô tả chi tiết của kịch bản.
     - **Thông số tùy chỉnh:** Có thể tùy chỉnh được các thông số đầu vào cho kịch bản đó ngay phần đầu màn hình.
     - **Payload:** Ở phía cuối màn hình bên phải, tester có thể xem được payload sẽ được gửi đi test (hỗ trợ tính năng xem trước toàn bộ payload).

4. **Gửi điện văn và ghi nhận kết quả (Đánh giá PASS/FAIL):**
   - Sau khi thiết lập xong, bấm nút **gửi điện văn** (Send / Execute) để thực thi.
   - Giao diện đánh giá **PASS/FAIL** của subcase sẽ hiện lên. Tester sẽ dựa vào kết quả thực tế để chọn **PASS** hoặc **FAIL** cho subcase đó, đồng thời có thể thêm **comment (cmt)** giải thích và lưu lại kết quả của từng subcase.
   - Ngoài việc đánh giá từng subcase, tester cũng có thể đánh **PASS/FAIL** tổng thể cho cả một Case.

5. **Xem trước và xuất báo cáo (Excel):**
   - Tester có thể xem trước kết quả đã đánh giá thông qua nút **Preview**.
   - Bấm xuất báo cáo dưới dạng file Excel. Báo cáo này sẽ tổng hợp đầy đủ các case/subcase đã test, kèm comment, và đặc biệt có đầy đủ **ngày giờ bắt đầu** và **ngày giờ kết thúc** đã được ghi nhận từ nút Recording.

## 5. Tùy biến cấu hình Payload nâng cao
Tool nạp danh sách case từ [cases.json](file:///workspace/cases.json) qua `TestbookLoader`.
- Công cụ hỗ trợ payload mặc định theo chuẩn nằm trong: `config/default_case_payloads.xml`.
- Khi tester thay đổi nội dung thông số hoặc payload trên UI và lưu, file `config/case_payloads.json` sẽ được tạo/cập nhật để ghi đè payload mặc định.
...
## 7. Gợi ý vận hành báo cáo
- Nên tạo 1 báo cáo cho 1 phiên kiểm thử (theo ngày hoặc vòng nghiệm thu).
- Đặt tên file báo cáo rõ ràng (ví dụ: `AMHS_SWIM_Test_Report_YYYYMMDD.xlsx`).
